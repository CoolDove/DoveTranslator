...
std::string fn_conv_to_utf8(std::string str) {
    int size = MultiByteToWideChar(CP_ACP, MB_COMPOSITE, str.c_str(), str.length(), nullptr, 0);
    std::wstring u16_str(size, '\0');
    MultiByteToWideChar(CP_ACP, MB_COMPOSITE, str.c_str(), str.length(), &u16_str[0], size);

    int u8_size = WideCharToMultiByte(CP_UTF8, 0, u16_str.c_str(), u16_str.length(), nullptr, 0, nullptr, nullptr);
    std::string u8_str(u8_size,'\0');
    WideCharToMultiByte(CP_UTF8, 0, u16_str.c_str(), u16_str.length(), &u8_str[0], u8_size, nullptr, nullptr);
    return u8_str;
}
...
void fn_init() {
    ...
    std::cin >> g_secret_key;

    std::ofstream ofs(path,std::ios_base::out);
    if (ofs)
    {
        ofs << g_appid << std::endl;
        ofs << g_secret_key << std::endl;
...
void fn_read_config() {
    ...
    if (ifs)
    {
        std::getline(ifs, g_appid);
        std::getline(ifs, g_secret_key);
    }
    else {
        fn_init();
    }
